...
for (i=support/2; i<(dimy-support/2);i++)
	...
	{
		mmin=io[i*dimx+j]; mmax=io[i*dimx+j];
		for(l=(-support/2);l<=support/2;l++) //On viste le voisinnage du support 
			for(k=(-support/2);k<=(support/2);k++)
				if(io[(i+l)*dimx+j+k]<mmin) mmin=io[(i+l)*dimx+j+k]; //On maj min puis max
				else if(io[(i+l)*dimx+j+k]>mmax) mmax=io[(i+l)*dimx+j+k];

		ir[i*dimx+j]=mmax - mmin;		
	}
