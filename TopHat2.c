...
//TopHat2 = io - ouverture(io). Sert à faire ressortir les élmt filaires d'une image.
//Image fermée pluss claire que l'image ouverte
...
int *io,*ir, *itmp;
int nbg, support;
register long int i,j,k,l,mmax, mmin;
	...
	for(j=support/2;j<(dimx-support/2);j++)
	{
		...
		for(l=(-support/2);l<=support/2;l++) //On viste le voisinnage du support 
			for(k=(-support/2);k<=(support/2);k++)
				if(io[(i+l)*dimx+j+k]<mmin) mmin=io[(i+l)*dimx+j+k]; //On maj min
		...
		for(l=(-support/2);l<=support/2;l++) //On viste le voisinnage du support 
			for(k=(-support/2);k<=(support/2);k++)
				if(itmp[(i+l)*dimx+j+k]>mmax) mmax=itmp[(i+l)*dimx+j+k]; //On maj max

		ir[i*dimx+j]=mmax;	
		ir[i*dimx+j]= io[i*dimx+j] - ir[i*dimx+j];
